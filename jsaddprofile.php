...
if(ISSET($_POST['add'])){
        ...
        $sql="INSERT INTO jsprofile(jsname,phonenumber,email,gender,county,town,dob,id_passport,postal,zip) VALUES('$jsname','$phonenumber','$email','$gender','$county','$town','$dob','$id_passport','$postal','$zip')" or die(mysqli_error());

    
        $result= mysqli_query($conn, $sql);
        if($result){
            $subject = "Profile Created";
            $message = "You have fully created your profile.";
            $sender = "From: cheruiyotemmanuel24@gmail.com";
            if(mail($email, $subject, $message, $sender)){
                $_SESSION['email'] = $email;
                echo("<script language='javascript'>window.alert('Profile Added successfully!');window.location='jslogin.php';</script>");
...
<div class="section">
    <form method="POST" action="jsaddprofile.php" id="profile" name="profile" onsubmit="return validateForm()"> 
        ...
        <fieldset style="width:800px; height: 1800px; background-color:floralwhite;">
            ...
            <input style="height:50px; width:300px; border: 1px hidden; background-color: black; color: floralwhite; font-size: 26px;" id="add" type="submit"  name="add" value="ADD PROFILE" onclick="validateForm()"><br>
        </fieldset>
        </form>
</div>
</body>
