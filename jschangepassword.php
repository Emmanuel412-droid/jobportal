...
include("database.php");
  ...
  if($old_password!="" && $password!="" && $confirm_pwd!="") :
    $session=$_SESSION["userjobseeker"];
  ...
  if($pwd == $c_pwd) :
  if($pwd!=$old_pwd) :
    $sql="SELECT * FROM `jsregister` WHERE  `password` ='$old_pwd' and jsname='$session'";
    $db_check=$conn->query($sql);
    $count=mysqli_num_rows($db_check);
  if($count==1) :
    $fetch=$conn->query("UPDATE `jsregister` SET `password` = '$pwd' where jsname='$session'");
...
<div class="sidenav">
    ...
    <a class="active" style="text-decoration: none; font-size: 22px;" href="jschangepassword.php">CHANGE PASSWORD</a><br>
...
<div class="section">
        <form method="POST" action="jschangepassword.php" id="changepassword" name="changepassword" onsubmit="return validateForm()">
