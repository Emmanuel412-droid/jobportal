 ...
 //if user click change password button
    if(isset($_POST['change'])){
        ...
        if($password !== $conpassword){
            ...
            $update_pass = "UPDATE jsregister SET code = $code, password = md5('$password') WHERE email = '$email'";
            $run_query = mysqli_query($conn, $update_pass);
            if($run_query){
                $info = "Your password changed. Now you can login with your new password.";
                $_SESSION['info'] = $info;
                echo("<script language='javascript'>window.alert('Password changed successfully!');window.location='jslogin.php';</script>");
             // header('Location: jslogin.php');
            }else{
                echo("<script language='javascript'>window.alert('Failed to change your password!');window.location='jsresetpassword.php';</script>");
...
<div class="section">
        <form method="POST" action="jsresetpassword.php" id="changepassword" name="changepassword" onsubmit="return validateForm()">
