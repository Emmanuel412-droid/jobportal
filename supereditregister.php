...
include("database.php");
    if(ISSET($_POST['edit'])){
      ...
      $sql = "UPDATE superadmin SET aname = '$aname',phonenumber = '$phonenumber',email='$email',gender='$gender',county='$county' WHERE aname='$aname'";
      ...
      $result= mysqli_query($conn, $sql);
        if($result){
            ...
            if(mail($email, $subject, $message, $sender)){
                ...
                echo("<script language='javascript'>window.alert('Details edited successfully!');window.location='superaccountdetails.php';</script>");
                
            }else{
                echo("<script language='javascript'>window.alert('Edit unsuccessful!');window.location='superaccountdetails.php';</script>");
            }
        }else{
            echo("<script language='javascript'>window.alert('Failed while inserting data into database!');window.location='superaccountdetails.php';</script>");
        }
    }
?>
<!DOCTYPE html>
...
ul {
    ...
    ul li {
        ...
        function validateForm() {
            let aname = document.forms["register"]["aname"].value;
            ...
            if (aname == ""  || phonenumber=="" || email=="" || gender=="" || county=="" ) {
                alert("Form cannot be blank.Enter details");
                return false;
            }
            else if(!regFname.test(aname)){
...
<div class="section">
<div class="sect">
    <form id="register" action="supereditregister.php" method="post" onsubmit="return validateForm()">
        <fieldset style="width:900px; background-color: floralwhite;">
            <legend><h1>Personal Details</h1></legend>

            <h2><label class="required">Admin Name</label></h2>
            <p style="color:red;"><small>Admin Name should be same as original one</small></p>
            <input style="height:50px; width:600px; border: 1px hidden; background-color: lightgrey; color: black; font-size: 26px;" width="40px" id="aname" type="text" name="aname" onselectstart="return false" oncut="return false" oncopy="return false" onpaste="return false" ondrag="return false" ondrop="return false" required="">
            ...
            <input style="height:50px; width:200px; border: 1px hidden; background-color: black; color: floralwhite; font-size: 28px;" id="edit" type="submit"  name="edit" value="EDIT" onclick="validateForm()">
